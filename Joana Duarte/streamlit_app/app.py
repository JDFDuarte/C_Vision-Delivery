...
# save_directory = "C:/Users/joana/OneDrive/Desktop/HSLU/3rd_semester/CV/c_vision_ocr/code/trocr-base-stage1-local"
...
# #             model = VisionEncoderDecoderModel.from_pretrained("microsoft/trocr-large-handwritten", ignore_mismatched_sizes=True)
...
def draw_bounding_boxes(image, boxes):
    draw = ImageDraw.Draw(image)
    for box in boxes:
        draw.polygon([tuple(point) for point in box], outline="green", width=2)
    return image
    ...
    with col3:
        ...
        if st.button("Extract and Calculate"):
            if st.session_state.current_image is not None:
                try:
                    extracted_text, bounding_boxes = predict_text_with_boxes(st.session_state.current_image)
                    result_image = draw_bounding_boxes(st.session_state.current_image.copy(), bounding_boxes)
                    st.session_state.extracted_text = extracted_text
